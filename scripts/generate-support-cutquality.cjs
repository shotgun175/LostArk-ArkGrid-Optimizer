...
//   node scripts/generate-support-cutquality.cjs
// Self-validates against per-bucket score ceilings; throws if any is exceeded.
const fs = require('fs');
const path = require('path');

const OUT_PATH = path.join(__dirname, '..', 'src', 'lib', 'cutplan', 'supportCutQuality.json');
...
const POOLS = {
  8: ['AtkPower', 'AddDamage', 'BrandPower', 'AllyDamageEnh'],
  9: ['AtkPower', 'BossDamage', 'AllyDamageEnh', 'AllyAttackEnh'],
  10: ['AddDamage', 'BossDamage', 'BrandPower', 'AllyAttackEnh'],
};
const TURNS = { UC: 5, R: 7, E: 9 };
const ARCH = [
  ['UC', 8],
  ['UC', 9],
  ['UC', 10],
...
const wTerm = (req) => (4 - req) * 2.4;
const pTerm = (pt) => (pt - 4) * 5.14;
const scoreState = (s, c) => wTerm(s.req) + pTerm(s.pt) + c[s.o1] * s.lv1 + c[s.o2] * s.lv2;

function seed(cost, rnd) {
...
function processOne(cost, rarity, c, rnd) {
...
const ceil = {}; // archetype -> bucket -> max score seen (ceiling check)

for (let i = 0; i < ARCH.length; i++) {
  const [rarity, cost] = ARCH[i];
  const key = ARCH_KEYS[i];
  const counts = {}; // bucket -> { total, above: {bl: n} }
  for (let n = 0; n < N; n++) {
...
// Ceiling gate: no bucket may EXCEED its theoretical ceiling (= 7.54 + topActiveCoeffs*5).
function ceilOf(cost, bucket) {
  const actives = POOLS[cost]
    .filter((o) => SUP[o] > 0)
    .map((o) => SUP[o])
    .sort((a, b) => b - a);
  const base = 7.54; // = wTerm(REQ_MIN) + pTerm(PT_MAX) = (4-3)*2.4 + (5-4)*5.14
